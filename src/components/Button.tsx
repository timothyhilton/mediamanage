...
interface ButtonProps{
    text: string
    link: string
    onClick?: any // todo: find the right thing for this
    className?: string
    arrow?: boolean
...
export default function Button({ text, link, onClick, className, arrow, textClassName }: ButtonProps){
    return(
        <Link to={link}>
            <button 
                onClick={onClick}
            ...
            >
                <div className="flex flex-row gap-x-4 w-fit">
                    <p className={`font-semibold whitespace-nowrap self-center ${textClassName}`}>{text}</p>
                    {arrow && <SVGArrow className="w-[1.5rem]" />}
                </div>
            </button>
        </Link>
    );
